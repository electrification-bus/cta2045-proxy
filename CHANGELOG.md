...
- CLI: `--log-level` (defaulting to `$LOG_LEVEL`, then INFO) and `--version` flags, for parity with the sibling proxies, plus a documented usage block and exit-code contract in the README.
...
- Specification provenance: a [`.ebus-spec.json`](.ebus-spec.json) lockfile records the eBus specification state this publisher builds against, and a `spec drift` CI workflow (with a README badge) flags when a pinned artifact moves past the synced commit.
