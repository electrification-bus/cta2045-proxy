...
- Graceful shutdown now uses ebus-sdk 0.10's `Device.stop()`: a bounded, tree-level teardown that publishes a final `$state=disconnected` for the bridge (covering the water-heater child per the Homie 5 effective-state rule) then stops the shared MQTT client, so a dead broker cannot stall shutdown. Replaces the previous set-LOST-then-`mqttc.stop()` teardown. The direct `ebus-mqtt-client` floor is raised to `>=0.1.7` to match what is pulled transitively. (CTAP-cbs)
- Packaging: the version now has a single source of truth (`__version__` in [`src/cta2045_proxy/__init__.py`](src/cta2045_proxy/__init__.py)), read dynamically by `pyproject.toml` and by the legacy `setup.py` shim, and the publish workflow refuses to release when the pushed git tag does not match the packaged version.

### Added

- CLI: `--log-level` (defaulting to `$LOG_LEVEL`, then INFO) and `--version` flags, for parity with the sibling proxies, plus a documented usage block and exit-code contract in the README. (CTAP-6u4)
...
- `flex/request` now accepts the Matter-aligned refinements: an advisory `cause` (`LOCAL_OPTIMIZATION` / `GRID_OPTIMIZATION`, carried through to `active-request`; CTA-2045 cannot enforce a directional opt-out) and, for load-up, a `target-percentage` that is encoded as CTA-2045 Advanced Load Up. Because Advanced Load Up carries the *extra* watt-hours to store, the absolute soc target is converted using the device's reported `soc/total-energy-storage` and `soc/soe` (`(target% / 100) * capacity - current`), falling back to Basic DR load-up when either is not yet known. The request `$format` is now strict (`additionalProperties: false`): `temporary-setpoint` is deliberately omitted, since Advanced Load Up quantifies energy in watt-hours and cannot express a temperature target. Not yet validated against live hardware; a rig check is tracked in [#1](https://github.com/electrification-bus/cta2045-proxy/issues/1).
- Specification provenance: a [`.ebus-spec.json`](.ebus-spec.json) lockfile records the eBus specification state this publisher builds against, and a `spec drift` CI workflow (with a README badge) flags when a pinned artifact moves past the synced commit. (CTAP-vlb)
